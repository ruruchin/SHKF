(function () {
  ...
  function defaultCenteredImageRegion(preset) {
    ...
    const ratio = w / h;
    if (ratio > 1.6) {
      // Широкий (напр. 1200×560): крупно по центру, с небольшими полями.
      return { x: 0.08, y: 0.06, w: 0.84, h: 0.88 };
    }
    if (h / w > 1.15) {
      // Высокий (напр. 360×488): крупный блок в нижней части кадра.
      return { x: 0.03, y: 0.28, w: 0.94, h: 0.68 };
    }
    // Близко к квадрату: крупно по центру.
